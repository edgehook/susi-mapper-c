...
int susi_on_connected(void* context){
	int ret;
...
static susi_device* find_device(devices_manager* mgr, char* device_id){
	if(!device_id) return NULL;

	return list_find_v2(mgr->devices, device_id, compare_device_id);	
}

static int remove_device(devices_manager* mgr, susi_device* dev){
...
int add_device_from_device_meta(devices_manager* mgr, device_spec_meta* dev_spec){
	...
	dev = create_susi_device(dev_spec);
	if(!dev){
...
*/
int start_device(devices_manager* mgr, char* dev_id){
	susi_device* dev = find_device(mgr, dev_id);

	if(!dev) return -1;

	return start_susi_device(dev);
}
...
static int susi_life_control(char* action, devices_spec_meta* devs_spec){
	int ret = 0;

	if(!action || !devs_spec) return 1;

	infof("susi_life_control action =%s \r\n", action);
...
static int susi_update_desired_twins(device_desired_twins_update_msg* update_msg){
	infof("susi_update_desired_twins update twins \r\n");

	return 0;
}

static void susi_keep_alive(void){
	int count, i = 0;
	...
	while(list_next_node(dev_mgr.devices, &current) != NULL){
		dev = (susi_device*)current->content;
		if(!dev) continue;
...
void main(int argc, char *argv[]){
	...
	* keepalive time: 5s
	*/
	ret = mapper_core_init("tcp://127.0.0.1:1884", NULL, NULL, "SUSI-API", 50, 5000);
	if(ret){
		errorf("mapper core init failed %d \r\n", ret);
		return ;
	}
	/* 2. Add setup  callback. */
	mapper_core_setup(susi_on_connected, susi_life_control,
				susi_update_desired_twins, susi_keep_alive);
